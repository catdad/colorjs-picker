...
var ready = function(){
    ...
    var selector = document.querySelector('.t-hue-selector');
    var colorPreview = document.querySelector('.t-color');
    
    var sat = document.querySelector('.t-picker-saturation');
    var satSelector = document.querySelector('.t-saturation-selector');
    
    // let's assume this won't change
    var pickerBB = picker.getBoundingClientRect();
    var satBB = sat.getBoundingClientRect();
    ...
    function setColor(){
        ...
        colorPreview.style.background = color.CSS();
        colorPreview.innerHTML = color.CSS();
        ...
        sat.style.backgroundImage = saturationGradientTemplate.replace('{{from}}', fromColor.CSS()).replace('{{to}}', toColor.CSS());
    }
    
    function setSelectorLocation(ev){
        var x = ev.layerX || ev.clientX - pickerBB.left;
        var y = ev.layerY || ev.clientY - pickerBB.top;
        
        if (x > pickerBB.width) x = pickerBB.width;
        else if (x < 0) x = 0;
        if (y > pickerBB.height) y = pickerBB.height;
        else if (y < 0) y = 0;
        
        h = (x/pickerBB.width) * 360;
        v = (y/pickerBB.height) * 100;
    ...
    function setSaturationSelector(ev){
        ...
        if (y > satBB.height) y = satBB.height;
        else if (y < 0) y = 0;
        
        s = (y/satBB.height) * 100;
        
        setColor();
        
        satSelector.style.transform = translateTemplate.replace('{{x}}', -2).replace('{{y}}', y - 5);
    }
...
//            setSelectorLocation(ev);
    ...
    function onmousedownSat(ev){
        ...
        var onmouseup = function(ev){
            sat.removeEventListener('mousemove', onmousemove);
            sat.removeEventListener('mouseup', onmouseup);
        };
        
        sat.addEventListener('mousemove', onmousemove);
        sat.addEventListener('mouseup', onmouseup);
    }
    
    sat.addEventListener('mousedown', onmousedownSat);
